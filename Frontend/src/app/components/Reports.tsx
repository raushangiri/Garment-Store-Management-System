...
import { Download, TrendingUp, Package, ShoppingBag, IndianRupee, User } from 'lucide-react';
import { useState, useMemo } from 'react';

export function Reports() {
  const { products, sales } = useStore();
  const { salesUsers } = useUserManagement();
  ...
  // Filter sales by date and sales person
  const filteredSales = useMemo(() => {
    return sales.filter(sale => {
      const withinDateRange = isWithinInterval(new Date(sale.date), { 
        start: startDate, 
        end: new Date() 
      });

      // If current user is sales person, only show their sales
      if (currentUser?.role === 'salesPerson') {
        return withinDateRange && sale.salesPersonId === currentUser.id;
      }

      // If admin selected a specific sales person
      if (selectedSalesPerson !== 'all') {
        return withinDateRange && sale.salesPersonId === selectedSalesPerson;
      }

      return withinDateRange;
    });
  }, [sales, startDate, selectedSalesPerson, currentUser]);

  // Sales over time
  const salesData = useMemo(() => {
    return Array.from({ length: daysAgo }, (_, i) => {
  ...
  }, [daysAgo, filteredSales]);

  // Sales by Sales Person (for admin)
  const salesPersonStats = useMemo(() => {
    ...
    filteredSales.forEach(sale => {
      const personId = sale.salesPersonId || 'unknown';
      const personName = sale.salesPersonName || 'Unknown';

      if (!stats.has(personId)) {
        stats.set(personId, {
          id: personId,
          name: personName,
          totalSales: 0,
      ...
      const stat = stats.get(personId);
      stat.totalSales += 1;
      stat.totalRevenue += sale.total;
      stat.itemsSold += sale.items.reduce((sum, item) => sum + item.quantity, 0);
    });
  ...
  const handleExport = () => {
    const csvData = [
      ['Date', 'Invoice Number', 'Sales Person', 'Items', 'Payment Method', 'Total'],
  ...
  return (
    <div className="space-y-6">
      ...
      {currentUser?.role === 'admin' && salesPersonStats.length > 0 && (
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-700">Sales Person</th>
                  <th className="text-right py-3 px-4 text-sm font-semibold text-slate-700">Total Sales</th>
                  <th className="text-right py-3 px-4 text-sm font-semibold text-slate-700">Items Sold</th>
              ...
              <tbody>
                {salesPersonStats.map((stat, index) => (
                  <tr key={stat.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="py-3 px-4 text-right text-slate-700">{stat.totalSales}</td>
                    <td className="py-3 px-4 text-right text-slate-700">{stat.itemsSold}</td>
                    <td className="py-3 px-4 text-right font-semibold text-green-600">
                      ₹{stat.totalRevenue.toLocaleString('en-IN')}
                    </td>
                    <td className="py-3 px-4 text-right text-slate-700">
                      ₹{(stat.totalRevenue / stat.totalSales).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                    </td>
                  </tr>
                ))}
              </tbody>
