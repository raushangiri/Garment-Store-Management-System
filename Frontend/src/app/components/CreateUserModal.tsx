...
export function CreateUserModal({ isOpen, onClose }: CreateUserModalProps) {
  const { addSalesUser, salesUsers } = useUserManagement();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (salesUsers.find(u => u.email === formData.email)) {
      toast.error('Email already exists');
      return;
    }

    addSalesUser(formData);
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full my-8 shadow-2xl">
        <div className="sticky top-0 bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-white px-6 py-4 flex items-center justify-between rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-bold">Add New Sales User</h2>
          </div>
          <button
            onClick={handleClose}
            ...
            <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-6 border border-purple-200 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors ${
                    formData.permissions.canDiscount ? 'bg-purple-500' : 'bg-slate-300'
                  }`}
                >
              ...
              <div className="flex items-center justify-between pt-3 border-t border-purple-200">
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors ${
                    formData.permissions.canRefund ? 'bg-purple-500' : 'bg-slate-300'
                  }`}
                >
                  <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${
              ...
              <div className="flex items-center justify-between pt-3 border-t border-purple-200">
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors ${
                    formData.permissions.canViewReports ? 'bg-purple-500' : 'bg-slate-300'
                  }`}
                >
                  <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${
